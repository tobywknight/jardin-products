...
//Add connect to Google Firestore
const Firestore = require('@google-cloud/firestore');

const db = new Firestore({
    projectId: 'jardin-2019-09-21',
...
// GET single item by product ID allows one product to be retrieved.
// TODO -- Need to add error handling for GET for unknown product ID!!!
app.get('/api/product/:id', function(req,res){
    var id = (req.params.id).toString();

    let prpductRef = db.collection('products').doc(id);
    let getDoc = prpductRef.get()
      .then(doc => {
        if (!doc.exists) {
            ...
            console.log("GetItem succeeded:", JSON.stringify(doc.data(), null, 2));
...
app.get('/api/products', function(req, res){
    ...
    let allProducts = productsRef.get()
        .then(snapshot => {
...
app.post('/api/product', function(req, res){
    ...
    */
   let docRef = db.collection('products').doc(newID.toString());
   let myDoc = docRef.set({
       productName: req.body.productName,
       category: req.body.category,
       description: req.body.description,
